<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否购买" prop="shifougoumai">
          <el-select v-model="ruleForm.shifougoumai" placeholder="请选择是否购买">
            <el-option
                v-for="(item,index) in shifougoumaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否购买" prop="shifougoumai">
	      <el-input v-model="ruleForm.shifougoumai"
                placeholder="是否购买" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xiangmugoumai/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiangmugoumaiCrossAddOrUpdateFlag = false;
                  this.parent.search();
